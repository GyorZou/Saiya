...
+ (void)showToast:(NSString *)message {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([toast.text length] > 0) {
            [NWF_APP_WINDOW addSubview:toast];

            // get the window frame to determine placement
            CGRect windowFrame = NWF_APP_WINDOW.frame;
...
+ (void)showProgress:(NSString *)message {
    if ([NSThread isMainThread]) {
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:NWF_APP_WINDOW animated:YES];
...
+ (void)dismissProgress {
    dispatch_async(dispatch_get_main_queue(), ^{
    [MBProgressHUD hideHUDForView:NWF_APP_WINDOW animated:YES];
    });
}
