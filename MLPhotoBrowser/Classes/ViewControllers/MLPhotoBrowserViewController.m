...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        [deleleBtn setImage:[UIImage ml_imageFromBundleNamed:@"ico-clear.png"] forState:UIControlStateNormal];
...
-(void)setPageLabelPage:(NSInteger)page{
    ...
    if (self.currentPage +1 > self.photos.count) {
...
- (UIPageControl *)pageControl{
    ...
    if (_pageControl == nil) {
        ...
        _pageControl.center = CGPointMake(size.width/2, size.height * 0.9);
...
- (void)showToView{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        if (self.status == UIViewAnimationAnimationStatusFade){
            ...
            CGRect rect = [self setMaxMinZoomScalesForCurrentBounds:imageView];
//            rect.origin.x = 0;
//            rect.origin.y = 0;
//            mainView.alpha = 1.0;
            imageView.frame = rect;//[self setMaxMinZoomScalesForCurrentBounds:imageView];
        }
    } completion:^(BOOL finished) {
